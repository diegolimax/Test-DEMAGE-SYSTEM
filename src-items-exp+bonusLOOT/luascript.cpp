...
void LuaInterface::setField(lua_State* L, const char* index, int64_t val)
{
...
void LuaInterface::registerFunctions()
{
	...
	//playerTalkToHimself(cid, boolean)
	lua_register(m_luaState, "playerTalkToHimself", LuaInterface::luaPlayerTalkToHimself);
	...
	lua_register(m_luaState, "doPlayerRemoveMoneySrc", LuaInterface::luaDoPlayerRemoveMoney);
	...
	//setMonsterName(cid, name, description)
	lua_register(m_luaState, "setMonsterName", LuaInterface::luaSetMonsterName);
	...
	//doItemSetPeriod(uid, seconds)
	lua_register(m_luaState, "doItemSetPeriod", LuaInterface::luaDoItemSetPeriod);
	...
	//getExperienceStage(level[, multiplier[, reset]])
	...
	//updateDailyMonster
	lua_register(m_luaState, "updateDailyMonster", LuaInterface::luaUpdateDailyMonster);
	...
	//playingEvent(cid, eventId)
	lua_register(m_luaState, "playingEvent", LuaInterface::luaPlayerPlayingEvent);

	registerEnum(FUNEVENT_NONE)
	registerEnum(FUNEVENT_BOMBERMAN)
	registerEnum(FUNEVENT_CAPTURETHEFLAG)
	registerEnum(FUNEVENT_BATTLEFIELD)
	registerEnum(FUNEVENT_BATTLEGROUNDS)
	registerEnum(FUNEVENT_DESERTWAR)
	registerEnum(FUNEVENT_ZOMBIE)
	registerEnum(FUNEVENT_SAFEZONE)
	registerEnum(FUNEVENT_BLIZZARD)
	registerEnum(FUNEVENT_ASSASSINATION)
	registerEnum(FUNEVENT_FIRESTORM)
	registerEnum(FUNEVENT_SNOWBALL)
	registerEnum(FUNEVENT_DEFENDTHETOWERS)
	registerEnum(FUNEVENT_INFECTED)
	registerEnum(FUNEVENT_BATTLE_ROYALE)
	registerEnum(FUNEVENT_COMMON_001)
	registerEnum(FUNEVENT_COMMON_002)
	registerEnum(FUNEVENT_COMMON_003)
	registerEnum(FUNEVENT_COMMON_004)
	registerEnum(FUNEVENT_COMMON_005)
	registerEnum(FUNEVENT_COMMON_006)
	registerEnum(FUNEVENT_COMMON_007)
	registerEnum(FUNEVENT_COMMON_008)
	registerEnum(FUNEVENT_COMMON_009)
	registerEnum(FUNEVENT_COMMON_010)

	registerEnum(LOOT_DROP_FULL)
	registerEnum(LOOT_DROP_PREVENT)
	registerEnum(LOOT_DROP_NONE)
	...
	lua_register(m_luaState, "setPlayerSkillLevel", LuaInterface::luaSetPlayerSkillLevel);
	lua_register(m_luaState, "getPlayerFistResets", LuaInterface::luaGetPlayerFistResets);
	lua_register(m_luaState, "setPlayerFistResets", LuaInterface::luaSetPlayerFistResets);

	lua_register(m_luaState, "isAutoSellLootEnabled", LuaInterface::luaIsAutoSellLootEnabled);
	lua_register(m_luaState, "setAutoSellLootEnabled", LuaInterface::luaSetAutoSellLootEnabled);

	lua_register(m_luaState, "getOutfitInfo", LuaInterface::luaGetOutfitInfo);

	lua_register(m_luaState, "doCreatureSetDropLootType", LuaInterface::luaDoCreatureSetDropLootType);
	lua_register(m_luaState, "getCreatureDropLootType", LuaInterface::luaGetCreatureDropLootType);
}
...
{
	//getThingDescription(uid[, distance = -1])
	int32_t distance = -1;
	if (lua_gettop(L) > 1) {
		distance = popNumber(L);
	}

	if (Thing* thing = getEnv()->getThingByUID(popNumber(L))) {
		lua_pushstring(L, thing->getDescription(distance).c_str());
	} else {
...
int32_t LuaInterface::luaSetPlayerSkillLevel(lua_State* L)
{
	//setPlayerSkillLevel(pid, skill, level)
	...
	if (skill < SKILL_FIRST || (skill > SKILL_LAST && skill != SKILL__MAGLEVEL)) {
		errorEx("Invalid skill type");
		lua_pushnil(L);
		return 1;
	}
	...
	if (skill == SKILL__MAGLEVEL) {
		...
		player->skills[skill][SKILL_LEVEL] = level;
		player->skills[skill][SKILL_TRIES] = 0;
		player->skills[skill][SKILL_PERCENT] = 0;
		player->sendSkills();
	}
...
int32_t LuaInterface::luaGetPlayerFistResets(lua_State* L)
{
	//getPlayerFistResets(cid)
	Player* player = getEnv()->getPlayerByUID(popNumber(L));
	if (player) {
		lua_pushnumber(L, player->getFistResets());
		return 1;
	}
...
int32_t LuaInterface::luaSetPlayerFistResets(lua_State* L)
{
	//setPlayerFistResets(cid, value)
	uint32_t value = static_cast<uint32_t>(popNumber(L));
	Player* player = getEnv()->getPlayerByUID(popNumber(L));
	if (player) {
		player->setFistResets(value);
		lua_pushboolean(L, 1);
		return 1;
	}
...
int32_t LuaInterface::luaIsAutoSellLootEnabled(lua_State* L)
{
	//isAutoSellLootEnabled(cid)
	Player* player = getEnv()->getPlayerByUID(popNumber(L));
	if (!player) {
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushnil(L);
	...
	lua_pushboolean(L, player->isAutoSellLootEnabled());
	return 1;
}

int32_t LuaInterface::luaSetAutoSellLootEnabled(lua_State* L)
{
	//setAutoSellLootEnabled(cid, enabled)
	bool enabled = popBoolean(L);
	Player* player = getEnv()->getPlayerByUID(popNumber(L));
	if (!player) {
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushnil(L);
	...
	player->setAutoSellLootEnabled(enabled);
	lua_pushboolean(L, 1);
	return 1;
}

int32_t LuaInterface::luaGetOutfitInfo(lua_State* L)
{
	//getOutfitInfo(lookType)
	...
	lua_createtable(L, 1, 69);
	setField(L, "id", outfit.outfitId);
	setField(L, "lookType", outfit.lookType);
	setField(L, "addons", outfit.addons);
	setField(L, "access", outfit.accessLevel);
	setField(L, "requirement", outfit.requirement);
	setField(L, "speed", outfit.speed);
	setField(L, "attackSpeed", outfit.attackSpeed);
	setField(L, "healthGain", outfit.healthGain);
	setField(L, "healthTicks", outfit.healthTicks);
	setField(L, "manaGain", outfit.manaGain);
	setField(L, "manaTicks", outfit.manaTicks);
	setField(L, "conditionSuppressions", outfit.conditionSuppressions);
	setField(L, "name", outfit.name);
	setField(L, "storageId", outfit.storageId);
	setField(L, "storageValue", outfit.storageValue);
	setFieldBool(L, "default", outfit.isDefault);
	setFieldBool(L, "premium", outfit.isPremium);
	setFieldBool(L, "manaShield", outfit.manaShield);
	setFieldBool(L, "invisible", outfit.invisible);
	setFieldBool(L, "regeneration", outfit.regeneration);

	setField(L, "skillFist", outfit.skills[SKILL_FIST]);
	setField(L, "skillClub", outfit.skills[SKILL_CLUB]);
	setField(L, "skillSword", outfit.skills[SKILL_SWORD]);
	setField(L, "skillAxe", outfit.skills[SKILL_AXE]);
	setField(L, "skillDist", outfit.skills[SKILL_DIST]);
	setField(L, "skillShield", outfit.skills[SKILL_SHIELD]);
	setField(L, "skillFish", outfit.skills[SKILL_FISH]);

	setField(L, "skillPercentFist", outfit.skillsPercent[SKILL_FIST]);
	setField(L, "skillPercentClub", outfit.skillsPercent[SKILL_CLUB]);
	setField(L, "skillPercentSword", outfit.skillsPercent[SKILL_SWORD]);
	setField(L, "skillPercentAxe", outfit.skillsPercent[SKILL_AXE]);
	setField(L, "skillPercentDist", outfit.skillsPercent[SKILL_DIST]);
	setField(L, "skillPercentShield", outfit.skillsPercent[SKILL_SHIELD]);
	setField(L, "skillPercentFish", outfit.skillsPercent[SKILL_FISH]);

	setField(L, "absorbPercentPhysical", outfit.absorb[COMBAT_PHYSICALDAMAGE]);
	setField(L, "absorbPercentEnergy", outfit.absorb[COMBAT_EARTHDAMAGE]);
	setField(L, "absorbPercentFire", outfit.absorb[COMBAT_FIREDAMAGE]);
	setField(L, "absorbPercentLifedrain", outfit.absorb[COMBAT_LIFEDRAIN]);
	setField(L, "absorbPercentManadrain", outfit.absorb[COMBAT_MANADRAIN]);
	setField(L, "absorbPercentDrown", outfit.absorb[COMBAT_DROWNDAMAGE]);
	setField(L, "absorbPercentIce", outfit.absorb[COMBAT_ICEDAMAGE]);
	setField(L, "absorbPercentHoly", outfit.absorb[COMBAT_HOLYDAMAGE]);
	setField(L, "absorbPercentDeath", outfit.absorb[COMBAT_DEATHDAMAGE]);

	setField(L, "reflectChancePhysical", outfit.reflect[REFLECT_CHANCE][COMBAT_PHYSICALDAMAGE]);
	setField(L, "reflectChanceEnergy", outfit.reflect[REFLECT_CHANCE][COMBAT_ENERGYDAMAGE]);
	setField(L, "reflectChanceEarth", outfit.reflect[REFLECT_CHANCE][COMBAT_EARTHDAMAGE]);
	setField(L, "reflectChanceFire", outfit.reflect[REFLECT_CHANCE][COMBAT_FIREDAMAGE]);
	setField(L, "reflectChanceLifedrain", outfit.reflect[REFLECT_CHANCE][COMBAT_LIFEDRAIN]);
	setField(L, "reflectChanceManadrain", outfit.reflect[REFLECT_CHANCE][COMBAT_MANADRAIN]);
	setField(L, "reflectChanceDrown", outfit.reflect[REFLECT_CHANCE][COMBAT_DROWNDAMAGE]);
	setField(L, "reflectChanceIce", outfit.reflect[REFLECT_CHANCE][COMBAT_ICEDAMAGE]);
	setField(L, "reflectChanceHoly", outfit.reflect[REFLECT_CHANCE][COMBAT_HOLYDAMAGE]);
	setField(L, "reflectChanceDeath", outfit.reflect[REFLECT_CHANCE][COMBAT_DEATHDAMAGE]);

	setField(L, "reflectPercentPhysical", outfit.reflect[REFLECT_PERCENT][COMBAT_PHYSICALDAMAGE]);
	setField(L, "reflectPercentEnergy", outfit.reflect[REFLECT_PERCENT][COMBAT_EARTHDAMAGE]);
	setField(L, "reflectPercentFire", outfit.reflect[REFLECT_PERCENT][COMBAT_FIREDAMAGE]);
	setField(L, "reflectPercentLifedrain", outfit.reflect[REFLECT_PERCENT][COMBAT_LIFEDRAIN]);
	setField(L, "reflectPercentManadrain", outfit.reflect[REFLECT_PERCENT][COMBAT_MANADRAIN]);
	setField(L, "reflectPercentDrown", outfit.reflect[REFLECT_PERCENT][COMBAT_DROWNDAMAGE]);
	setField(L, "reflectPercentIce", outfit.reflect[REFLECT_PERCENT][COMBAT_ICEDAMAGE]);
	setField(L, "reflectPercentHoly", outfit.reflect[REFLECT_PERCENT][COMBAT_HOLYDAMAGE]);
	setField(L, "reflectPercentDeath", outfit.reflect[REFLECT_PERCENT][COMBAT_DEATHDAMAGE]);

	setField(L, "statMaxHealth", outfit.stats[STAT_MAXHEALTH]);
	setField(L, "statMaxMana", outfit.stats[STAT_MAXMANA]);
	setField(L, "statSoul", outfit.stats[STAT_SOUL]);
	setField(L, "statMagLevel", outfit.stats[STAT_MAGICLEVEL]);

	setField(L, "statPercentMaxHealth", outfit.statsPercent[STAT_MAXHEALTH]);
	setField(L, "statPercentMaxMana", outfit.statsPercent[STAT_MAXMANA]);
	setField(L, "statPercentSoul", outfit.statsPercent[STAT_SOUL]);
	setField(L, "statPercentMagLevel", outfit.statsPercent[STAT_MAGICLEVEL]);

	createTable(L, "groups", 0, outfit.groups.size());
	int32_t i = 0;
	for (int32_t groupId : outfit.groups) {
...
int32_t LuaInterface::luaDoCreatureSetDropLootType(lua_State* L)
{
	//doCreatureSetDropLootType(cid, dropType)
	lootDrop_t dropType = static_cast<lootDrop_t>(popNumber(L));
	if (dropType < LOOT_DROP_FULL || dropType > LOOT_DROP_NONE) {
		errorEx("Invalid dropType");
		lua_pushnil(L);
		return 1;
	}

	if (Creature* creature = getEnv()->getCreatureByUID(popNumber(L))) {
		creature->setDropLoot(dropType);
		lua_pushboolean(L, true);
	} else {
...
int32_t LuaInterface::luaGetCreatureDropLootType(lua_State* L)
{
	//getCreatureDropLootType(cid)
	if (Creature* creature = getEnv()->getCreatureByUID(popNumber(L))) {
		lua_pushnumber(L, creature->lootDrop);
	} else {
		errorEx(getError(LUA_ERROR_CREATURE_NOT_FOUND));
		lua_pushnil(L);
	}
...
int32_t LuaInterface::luaDoCreatureAddMana(lua_State* L)
{
	...
	int64_t manaChange = popNumber(L);
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
	{
		if(aggressive)
...
{
	//doPlayerRemoveMoneySrc(cid,money)
	uint64_t money = popNumber(L);
...
int32_t LuaInterface::luaAddDamageCondition(lua_State* L)
{
	...
	int64_t value = popNumber(L), time = popNumber(L), rounds = popNumber(L);
	ScriptEnviroment* env = getEnv();
...
int32_t LuaInterface::luaDoCombatAreaHealth(lua_State* L)
{
	...
	int64_t maxChange = popNumber(L), minChange = popNumber(L);
	uint32_t areaId = popNumber(L);
...
int32_t LuaInterface::luaSetMonsterName(lua_State* L)
{
    // setMonsterName(cid, newName, newDescription)
...
int32_t LuaInterface::luaPlayerTalkToHimself(lua_State* L)
{
	//playerTalkToHimself(cid, boolean)
	bool boolean = popBoolean(L);
	ScriptEnviroment* env = getEnv();
	if (Player* player = env->getPlayerByUID(popNumber(L))) {
		player->talkHimself = boolean;
		lua_pushboolean(L, true);
	} else {
		errorEx(getError(LUA_ERROR_PLAYER_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaInterface::luaGetCreatureMaxMana(lua_State* L)
{
	...
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
		lua_pushnumber(L, ignoreModifiers ? creature->manaMax : creature->getMaxMana());
	else
	{
		errorEx(getError(LUA_ERROR_CREATURE_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaInterface::luaGetCreatureMaxHealth(lua_State* L)
{
	...
	ScriptEnviroment* env = getEnv();
	if(Creature* creature = env->getCreatureByUID(popNumber(L)))
		lua_pushnumber(L, ignoreModifiers ? creature->healthMax : creature->getMaxHealth());
	else
	{
		errorEx(getError(LUA_ERROR_CREATURE_NOT_FOUND));
		lua_pushboolean(L, false);
...
int32_t LuaInterface::luaDoReloadInfo(lua_State* L)
{
	...
	else
	{
		if(id >= RELOAD_FIRST && id <= RELOAD_LAST)
			lua_pushboolean(L, g_game.reloadInfo((ReloadInfo_t)id, cid, false));
		else
			lua_pushboolean(L, false);
	}
...
int32_t LuaInterface::luaGetItemInfo(lua_State* L)
{
	...
	setField(L, "damagePercent", item->hasAbilities() ? item->abilities->damagePercent : 0);
	setField(L, "healingPercent", item->hasAbilities() ? item->abilities->healingPercent : 0);

	setField(L, "reflectChancePhysical", item->hasAbilities() ? item->abilities->reflect[REFLECT_CHANCE][COMBAT_PHYSICALDAMAGE] : 0);
	setField(L, "reflectChanceEnergy", item->hasAbilities() ? item->abilities->reflect[REFLECT_CHANCE][COMBAT_ENERGYDAMAGE] : 0);
	setField(L, "reflectChanceEarth", item->hasAbilities() ? item->abilities->reflect[REFLECT_CHANCE][COMBAT_EARTHDAMAGE] : 0);
	setField(L, "reflectChanceFire", item->hasAbilities() ? item->abilities->reflect[REFLECT_CHANCE][COMBAT_FIREDAMAGE] : 0);
	setField(L, "reflectChanceLifedrain", item->hasAbilities() ? item->abilities->reflect[REFLECT_CHANCE][COMBAT_LIFEDRAIN] : 0);
	setField(L, "reflectChanceManadrain", item->hasAbilities() ? item->abilities->reflect[REFLECT_CHANCE][COMBAT_MANADRAIN] : 0);
	setField(L, "reflectChanceDrown", item->hasAbilities() ? item->abilities->reflect[REFLECT_CHANCE][COMBAT_DROWNDAMAGE] : 0);
	setField(L, "reflectChanceIce", item->hasAbilities() ? item->abilities->reflect[REFLECT_CHANCE][COMBAT_ICEDAMAGE] : 0);
	setField(L, "reflectChanceHoly", item->hasAbilities() ? item->abilities->reflect[REFLECT_CHANCE][COMBAT_HOLYDAMAGE] : 0);
	setField(L, "reflectChanceDeath", item->hasAbilities() ? item->abilities->reflect[REFLECT_CHANCE][COMBAT_DEATHDAMAGE] : 0);

	setField(L, "reflectPercentPhysical", item->hasAbilities() ? item->abilities->reflect[REFLECT_PERCENT][COMBAT_PHYSICALDAMAGE] : 0);
	setField(L, "reflectPercentEnergy", item->hasAbilities() ? item->abilities->reflect[REFLECT_PERCENT][COMBAT_ENERGYDAMAGE] : 0);
	setField(L, "reflectPercentEarth", item->hasAbilities() ? item->abilities->reflect[REFLECT_PERCENT][COMBAT_EARTHDAMAGE] : 0);
	setField(L, "reflectPercentFire", item->hasAbilities() ? item->abilities->reflect[REFLECT_PERCENT][COMBAT_FIREDAMAGE] : 0);
	setField(L, "reflectPercentLifedrain", item->hasAbilities() ? item->abilities->reflect[REFLECT_PERCENT][COMBAT_LIFEDRAIN] : 0);
	setField(L, "reflectPercentManadrain", item->hasAbilities() ? item->abilities->reflect[REFLECT_PERCENT][COMBAT_MANADRAIN] : 0);
	setField(L, "reflectPercentDrown", item->hasAbilities() ? item->abilities->reflect[REFLECT_PERCENT][COMBAT_DROWNDAMAGE] : 0);
	setField(L, "reflectPercentIce", item->hasAbilities() ? item->abilities->reflect[REFLECT_PERCENT][COMBAT_ICEDAMAGE] : 0);
	setField(L, "reflectPercentHoly", item->hasAbilities() ? item->abilities->reflect[REFLECT_PERCENT][COMBAT_HOLYDAMAGE] : 0);
	setField(L, "reflectPercentDeath", item->hasAbilities() ? item->abilities->reflect[REFLECT_PERCENT][COMBAT_DEATHDAMAGE] : 0);
...
int32_t LuaInterface::luaDoItemSetPeriod(lua_State* L)
{
	//doItemSetPeriod(uid, period)
	uint32_t seconds = popNumber(L);
	ScriptEnviroment* env = getEnv();
	...
	item->setPeriod(time(nullptr) + seconds);
	lua_pushboolean(L, true);
	return 1;
}
...
int32_t LuaInterface::luaGetExperienceStage(lua_State* L)
{
	...
	uint32_t level = popNumber(L);
	lua_pushnumber(L, g_game.getExperienceStage(level, reset, multiplier));
	return 1;
}
...
{
	uint32_t number = ~static_cast<uint32_t>(popNumber(L));
	lua_pushnumber(L, number);
	return 1;
}

int32_t LuaInterface::luaUpdateDailyMonster(lua_State* L)
{
	...
	g_game.dmonster.name = asLowerCaseString(name);
	g_game.dmonster.type = type;
	g_game.dmonster.percent = percent;
	lua_pushboolean(L, true);
	return 1;
}
...
int32_t LuaInterface::luaPlayerPlayingEvent(lua_State* L)
{
	//playingEvent(cid, eventId)
	eventId_t eventId = FUNEVENT_NONE;
	if (lua_gettop(L) > 1) {
		eventId = static_cast<eventId_t>(popNumber(L));
	}

	ScriptEnviroment* env = getEnv();
	if (Player* player = env->getPlayerByUID(popNumber(L))) {
		if (lua_gettop(L) == 1) {
			lua_pushnumber(L, player->playingEvent);
		} else {
			player->playingEvent = eventId;
			lua_pushboolean(L, true);
		}
	} else {
		lua_pushnil(L);
...
std::string LuaInterface::getStackTrace(const std::string& error_desc)
{
	...
	lua_pushstring(m_luaState, error_desc.c_str());
