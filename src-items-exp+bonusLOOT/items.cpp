...
ItemType::ItemType()
{
	...
	movable = walkStack = usetimeBetweenActions = usetimeBetweenExActions = true;
	alwaysOnTopOrder = 0;
	rotateTo = 0;
...
void Items::parseItemNode(xmlNodePtr itemNode, uint32_t id)
{
	...
	for(xmlNodePtr itemAttributesNode = itemNode->children; itemAttributesNode; itemAttributesNode = itemAttributesNode->next)
	{
			...
			if(readXMLString(itemAttributesNode, "value", strValue))
			{
				...
				if(combatType != COMBAT_NONE)
				{
					...
					for(xmlNodePtr fieldAttributesNode = itemAttributesNode->children; fieldAttributesNode; fieldAttributesNode = fieldAttributesNode->next)
					{
						...
						{
							if(readXMLInteger64(fieldAttributesNode, "value", int64Value))
								start = std::max<int64_t>(0, int64Value);
						}
						...
						if(tmpStrValue == "damage")
						{
							if(readXMLInteger64(fieldAttributesNode, "value", int64Value))
							{
								...
								if(start > 0)
								{
									...
									for(std::list<int64_t>::iterator it = damageList.begin(); it != damageList.end(); ++it)
		...
		else if(tmpStrValue == "usetimebetweenactions")
		{
			if(readXMLInteger(itemAttributesNode, "value", intValue))
				it.usetimeBetweenActions = (intValue != 0);
		}
		else if(tmpStrValue == "usetimebetweenexactions")
		{
			if(readXMLInteger(itemAttributesNode, "value", intValue))
				it.usetimeBetweenExActions = intValue;
		}
		else if(tmpStrValue == "damagepercent")
		{
			if(readXMLInteger(itemAttributesNode, "value", intValue))
				it.getAbilities()->damagePercent = intValue;
		}
		else if(tmpStrValue == "healingpercent")
		{
			if(readXMLInteger(itemAttributesNode, "value", intValue))
				it.getAbilities()->healingPercent = intValue;
		}
		else if(tmpStrValue == "sellvalue")
		{
			if (readXMLInteger(itemAttributesNode, "value", intValue)) {
				it.sellValue = intValue;
			}
		}
		else
