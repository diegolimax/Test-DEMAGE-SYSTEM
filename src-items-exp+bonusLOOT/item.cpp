	...
	int32_t subType/* = -1*/, bool addArticle/* = true*/)
{
	...
	else if(it.weaponType != WEAPON_NONE)
	{
		...
		if(it.hasAbilities())
		{
			...
			if(it.abilities->stats[STAT_MAGICLEVEL])
			{
				...
				s << "magic level " << std::showpos << (int64_t)it.abilities->stats[STAT_MAGICLEVEL] << std::noshowpos;
			}
			...
			if (statPercent != 0) {
				...
				s << "Damage " << std::showpos << statPercent << std::noshowpos << '%';
			}

			statPercent = (item ? item->getHealingPercent() : it.abilities->healingPercent);
			if (statPercent != 0) {
				if (begin) {
					begin = false;
					s << " (";
				...
				s << "Healing " << std::showpos << statPercent << std::noshowpos << '%';
			}
			...
			if (it.abilities->healthGain) {
				...
				s << "health +" << it.abilities->healthGain << "/" << (it.abilities->healthTicks / 1000.0f) << "s";
			}
			...
			if (it.abilities->manaGain) {
				...
				s << "mana +" << it.abilities->manaGain << "/" << (it.abilities->manaTicks / 1000.0f) << "s";
			}
			...
			int32_t statPercent = item->getDamagePercent();
			if (statPercent != 0) {
				if (begin) {
					begin = false;
					s << " (";
			...
			statPercent = item->getHealingPercent();
			if (statPercent != 0) {
				if (begin) {
					begin = false;
					s << " (";
		...
		bool allSame = true;
		std::vector<int32_t> tmpValues;
		for (uint32_t i = (COMBAT_FIRST + 1); i <= COMBAT_LAST; i <<= 1) {
			if (i == COMBAT_UNDEFINEDDAMAGE || i == COMBAT_HEALING) {
				continue;
			}

			tmpValues.push_back(item ? item->getReflectChance(static_cast<CombatType_t>(i)) : (it.abilities ? it.abilities->reflect[REFLECT_CHANCE][i] : 0));
			if (tmpValues.size() > 1 && tmpValues[tmpValues.size() - 2] != tmpValues.back()) {
				allSame = false;
			}
		}
		...
		if (!allSame) {
			...
			for (uint32_t i = (COMBAT_FIRST + 1); i <= COMBAT_LAST; i <<= 1) {
				...
				int32_t chance = tmpValues[idx++];
				if (chance == 0) {
					continue;
				}

				int32_t percent = (item ? item->getReflectPercent(static_cast<CombatType_t>(i)) : (it.abilities ? it.abilities->reflect[REFLECT_PERCENT][i] : 0));
				if (percent == 0) {
					continue;
				}
				...
				s << ' ' << getCombatName(static_cast<CombatType_t>(i));
			}
			...
			for (uint32_t i = (COMBAT_FIRST + 1); i <= COMBAT_LAST; i <<= 1) {
				...
				tmpValues.push_back(item ? item->getReflectPercent(static_cast<CombatType_t>(i)) : it.abilities->reflect[REFLECT_PERCENT][i]);
				if (tmpValues.size() > 1 && tmpValues[tmpValues.size() - 2] != tmpValues.back()) {
					allSame = false;
					break;
				}
	...
	uint32_t period = item ? item->getPeriod() : 0;
	bool hasPeriod = period > 0;

	if(it.showDuration || hasPeriod)
	{
		int32_t duration = item ? item->getDuration() / 1000 : 0;
		if (hasPeriod) {
			duration = std::max<int32_t>(0, period - time(nullptr));
		}
	...
	if(it.wieldInfo)
	{
		...
		if(it.wieldInfo & WIELDINFO_RESET)
			s << " of reset " << (int32_t)it.minReqReset << " or higher";
	...
	if (item) {
		const std::string& ownerName = item->getOwnerName();
		if (!ownerName.empty()) {
			s << std::endl << "Owner: " << ownerName << ".";
		}
	}
...
void Item::setPeriod(uint32_t seconds)
{
	if (seconds == 0) {
		return;
	}
	...
	addRef();
	g_game.addPeriodItem(this);
	setAttribute("period", (int32_t)seconds);
}
