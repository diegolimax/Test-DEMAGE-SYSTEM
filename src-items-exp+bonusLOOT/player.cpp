...
{
	uint16_t coinId = shopOwner ? shopOwner->getCoinId() : 0;
	if(Item::items[itemId].worth || itemId == coinId)
	{
		sendGoods();
		return;
	}
...
int64_t Player::getPlayerInfo(playerinfo_t playerinfo) const
{
	switch(playerinfo)
	{
		...
		case PLAYERINFO_MAGICLEVEL:
			return std::max<int64_t>(0, ((int64_t)magLevel + varStats[STAT_MAGICLEVEL]));
		case PLAYERINFO_MAGICLEVELPERCENT:
			return magLevelPercent;
		case PLAYERINFO_HEALTH:
			return health;
		case PLAYERINFO_MAXHEALTH:
			return std::max<int64_t>(1, (healthMax + varStats[STAT_MAXHEALTH]));
		case PLAYERINFO_MANA:
			return mana;
		case PLAYERINFO_MAXMANA:
			return std::max<int64_t>(0, (manaMax + varStats[STAT_MAXMANA]));
		case PLAYERINFO_SOUL:
			return std::max<int64_t>(0, (soul + varStats[STAT_SOUL]));
		default:
			break;
	}
...
int64_t Player::getDefaultStats(stats_t stat)
{
	switch(stat)
	{
		case STAT_MAGICLEVEL:
			return (int64_t)getMagicLevel() - varStats[STAT_MAGICLEVEL];
		case STAT_MAXHEALTH:
			return getMaxHealth() - varStats[STAT_MAXHEALTH];
		case STAT_MAXMANA:
			return getMaxMana() - varStats[STAT_MAXMANA];
		case STAT_SOUL:
			return getSoul() - varStats[STAT_SOUL];
		default:
			break;
	}
...
bool Player::isMuted(uint16_t channelId, MessageClasses type, int32_t& time)
{
	...
	return time > 0 && type != MSG_NPC_TO && (type != MSG_CHANNEL || (channelId != CHANNEL_GUILD && !g_chat.isPrivateChannel(channelId)));
}
...
void Player::addExperience(uint64_t exp, Creature* source)
{
	...
	if (source) {
		g_events->executeGainExperience(this, exp);
	}

	if(exp == 0)
		return;

	if (g_game.dmonster.type == DAILYMONSTER_EXP && source) {
		Monster* monster = source->getMonster();
		if (monster) {
			if (asLowerCaseString(monster->getName()) == g_game.dmonster.name) {
				exp *= (1.0f + g_game.dmonster.percent / 100.0f);
			}
		}
	}
	...
	SpectatorVec list;
	g_game.getSpectators(list, targetPos, false, true, Map::maxViewportX, Map::maxViewportX, Map::maxViewportY, Map::maxViewportY);

	const std::string& formattedValue = formatValue(exp);
	g_game.addAnimatedText(list, targetPos, color, formattedValue);

	std::stringstream ss;
	ss << ucfirst(getNameDescription()) << " gained " << formattedValue << " experience points.";
	...
	g_game.addStatsMessage(textList, MSG_EXPERIENCE_OTHERS, ss.str(), targetPos);

	ss.str("");
	ss << "You gained " << formattedValue << " experience points.";
	sendStatsMessage(MSG_EXPERIENCE, ss.str(), targetPos);
...
void Player::removeExperience(uint64_t exp, bool updateStats/* = true*/)
{
	...
	while(level > 1 && experience < Player::getExpForLevel(level))
	{
		...
		healthMax = std::max((int64_t)0, (healthMax - (int64_t)voc->getGain(GAIN_HEALTH)));
		manaMax = std::max((int64_t)0, (manaMax - (int64_t)voc->getGain(GAIN_MANA)));
	...
	bool checkDefense/* = false*/, bool checkArmor/* = false*/, bool reflect/* = true*/, bool field/* = false*/, bool element/* = false*/)
{
	...
	if (attacker && !element) {
		uint8_t color = g_config.getNumber(ConfigManager::SQUARE_COLOR);
		if (color < 0) {
	...
	if (vocation->getMultiplier(MULTIPLIER_MAGICDEFENSE) != 1.0 && combatType != COMBAT_PHYSICALDAMAGE &&
		...
		damage -= (int64_t)std::ceil((double)(damage * vocation->getMultiplier(MULTIPLIER_MAGICDEFENSE)) / 100.);
	}
	...
	int32_t absorbPercent = 0;
	int32_t reflectPercent = 0;

	if (attacker != this) {
		for (uint8_t slot = SLOT_FIRST; slot < SLOT_LAST; ++slot) {
			Item* inventoryItem = getInventoryItem(static_cast<slots_t>(slot));
			if (!inventoryItem || inventoryItem->isRemoved() || (g_moveEvents->hasEquipEvent(inventoryItem) && !isItemAbilityEnabled(static_cast<slots_t>(slot)))) {
				continue;
			}

			bool transform = false;
			int32_t percentValue;
			const ItemType& it = Item::items[inventoryItem->getID()];
			if (it.hasAbilities()) {
				percentValue = it.abilities->absorb[combatType];
				if (percentValue != 0) {
					absorbPercent += percentValue;
					if (inventoryItem->hasCharges()) {
						transform = true;
					}
				}

				percentValue = (field ? it.abilities->fieldAbsorb[combatType] : 0);
				if (percentValue != 0) {
					absorbPercent += percentValue;
					if (inventoryItem->hasCharges()) {
						transform = true;
			...
			percentValue = (reflect ? inventoryItem->getReflectPercent(combatType) : 0);
			if (percentValue != 0 && inventoryItem->getReflectChance(combatType) >= random_range(1, 100)) {
				reflectPercent += percentValue;
				if (inventoryItem->hasCharges()) {
					transform = true;
				}
			}
			...
			if (!element && transform) {
				g_game.transformItem(inventoryItem, inventoryItem->getID(), std::max(0, static_cast<int32_t>(inventoryItem->getCharges()) - 1));
			}
		}
	}
	/*
	...
	*/
	...
	absorbPercent += vocation->getAbsorb(combatType);
	reflectPercent += (reflect ? vocation->getReflect(combatType) : 0);

	damage -= std::ceil((damage * absorbPercent) / 100.0);
	if (damage <= 0) {
		damage = 0;
		blockType = BLOCK_DEFENSE;
	}

	if (reflectPercent != 0 && !element) {
		int64_t reflectDamage = std::ceil((damage * reflectPercent) / 100.0);
		if (combatType != COMBAT_HEALING) {
			reflectDamage = -reflectDamage;
		}

		if (!g_game.combatBlockHit(combatType, this, attacker, reflectDamage, false, false, true, false)) {
			g_game.combatChangeHealth(combatType, NULL, attacker, reflectDamage);
		}
	}
	return blockType;
}
...
bool Player::gainExperience(double& gainExp, Creature* target)
{
	...
	for (auto it : getCreatureEvents(CREATURE_EVENT_GAINEXPERIENCE)) {
...
bool Player::rateExperience(double& gainExp, Creature* target)
{
	...
	gainExp *= rates[SKILL__LEVEL] * g_game.getExperienceStage(level, reset, vocation->getExperienceMultiplier());
...
uint32_t Player::getAttackSpeed() const
{
	...
	int32_t attackSpeed;
	if (_weapon && _weapon->getAttackSpeed() != 0) {
		attackSpeed = _weapon->getAttackSpeed() + modifiers;
	} else {
		attackSpeed = (vocation->getAttackSpeed() / std::max((size_t)1, getWeapons().size())) + modifiers;
	}

	int32_t fistResets = getFistResets();
	int32_t fistResetBonus = g_config.getNumber(ConfigManager::FIST_RESET_BONUS);
	if (fistResets != 0 && fistResetBonus > 0) {
		attackSpeed -= (attackSpeed * ((fistResets * fistResetBonus) / 100.0));
	}
	return std::max<int32_t>(attackSpeed, g_config.getNumber(ConfigManager::MIN_ATTACKSPEED));
}
	...
	switch(accountManager)
	{
		...
		case MANAGER_ACCOUNT:
		{
			...
			else if(talkState[6])
			{
				managerString = text;
				trimString(managerString);
				const MonsterType* mType = g_monsters.getMonsterType(managerString);
				...
				if(managerString.length() < 3)
					...
					msg << "Sorry, but you can't use a monster name. Try again.";
				else
				{
					std::string tmp = asLowerCaseString(managerString);
					if(tmp.substr(0, 4) != "god " && tmp.substr(0, 3) != "cm " && tmp.substr(0, 3) != "gm ")
...
void Player::increaseCombatValues(int64_t& min, int64_t& max, bool useCharges, bool countWeapon)
{
	if(min > 0)
		min *= vocation->getMultiplier(MULTIPLIER_HEALING);
	else
		min *= vocation->getMultiplier(MULTIPLIER_MAGIC);

	if(max > 0)
		max *= vocation->getMultiplier(MULTIPLIER_HEALING);
	else
		max *= vocation->getMultiplier(MULTIPLIER_MAGIC);

	Item* item = NULL;
	int64_t minValue = 0, maxValue = 0, i = SLOT_FIRST;
	for(; i < SLOT_LAST; ++i)
	{
		if(!(item = getInventoryItem((slots_t)i)) || item->isRemoved() ||
			(g_moveEvents->hasEquipEvent(item) && !isItemAbilityEnabled((slots_t)i)))
		...
		if(min > 0)
		{
			...
			if(it.abilities->increment[HEALING_PERCENT])
				min = std::ceil((min * it.abilities->increment[HEALING_PERCENT]) / 100.0);
		}
		else
		{
			minValue -= it.abilities->increment[MAGIC_VALUE];
			if(it.abilities->increment[MAGIC_PERCENT])
				min = std::ceil((min * it.abilities->increment[MAGIC_PERCENT]) / 100.0);
		}
		...
		if(max > 0)
		{
			...
			if(it.abilities->increment[HEALING_PERCENT])
				max = std::ceil((max * it.abilities->increment[HEALING_PERCENT]) / 100.0);
		}
		else
		{
			maxValue -= it.abilities->increment[MAGIC_VALUE];
			if(it.abilities->increment[MAGIC_PERCENT])
				max = std::ceil((max * it.abilities->increment[MAGIC_PERCENT]) / 100.0);
		}
