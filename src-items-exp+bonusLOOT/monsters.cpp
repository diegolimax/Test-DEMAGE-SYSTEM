...
{
	canPushItems = canPushCreatures = isSummonable = isIllusionable = isConvinceable = isLureable = isWalkable = hideName = hideHealth = eliminable  = ignoreSpawnBoost = false;
	...
	AutoLootStruct& ALS)
{
	...
	for(; it != lootBlock.childLoot.end() && !parent->full(); ++it)
	{
		...
		for(ItemList::iterator iit = items.begin(); iit != items.end(); ++iit)
		{
			...
			{
				if(createChildLoot(container, *it, ALS))
			...
			{
				if (ALS.enabled && ALS.owner->checkAutoLoot(tmpItem->getID())) {
					if (ALS.owner->isMoneyAutoLoot(tmpItem, ALS.money)) {
						delete tmpItem;
						continue;
					}

					int32_t sellValue = tmpItem->getSellValue();
					if (ALS.owner->isAutoSellLootEnabled() && sellValue > 0) {
						++ALS.itemsSold;
						ALS.soldValue += sellValue;
						delete tmpItem;
						continue;
					}

					g_game.internalPlayerAddItem(NULL, ALS.owner, tmpItem);
					if (ALS.textBegin) {
						ALS.ss << tmpItem->getNameDescription();
						ALS.textBegin = false;
					} else {
						ALS.ss << ", " << tmpItem->getNameDescription();
					}
...
void MonsterType::dropLoot(Container* corpse)
{
	...
	for(LootItems::const_iterator it = lootItems.begin(); it != lootItems.end() && !corpse->full(); ++it)
	{
		...
		for(ItemList::iterator iit = items.begin(); iit != items.end(); ++iit)
		{
			...
			if(Container* container = tmpItem->getContainer())
			{
				...
				if (ALS.enabled && owner->checkAutoLoot(tmpItem->getID())) {
					if (owner->isMoneyAutoLoot(tmpItem, ALS.money)) {
						delete tmpItem;
						continue;
					}

					int32_t sellValue = tmpItem->getSellValue();
					if (owner->isAutoSellLootEnabled() && sellValue > 0) {
						++ALS.itemsSold;
						ALS.soldValue += sellValue;
						delete tmpItem;
						continue;
	...
	if (ALS.money != 0) {
		if (owner->statusAutoMoneyCollect() == "Bank") {
			owner->balance += ALS.money;
		} else {
			g_game.addMoney(owner, ALS.money);
		}

		if (ALS.textBegin) {
			ALS.ss << ALS.money << "x gold coins";
			ALS.textBegin = false;
		} else {
			ALS.ss << ", " << ALS.money << "x gold coins";
		}
	...
	if (ALS.enabled) {
		ss << "\nAutoLoot Colleted: " << ALS.ss.str() << '.';
		if (ALS.itemsSold > 0) {
			if (g_config.getBool(ConfigManager::AUTOSELL_TO_BANK)) {
				owner->balance += ALS.soldValue;
			} else {
				g_game.addMoney(owner, ALS.soldValue);
			}
			ss << "\nAutoLoot sold " << ALS.itemsSold << " item" << (ALS.itemsSold > 1 ? "s" : "") << " (" << ALS.soldValue << "gps).";
		}
	}
...
ConditionDamage* Monsters::getDamageCondition(ConditionType_t conditionType,
	int64_t maxDamage, int64_t minDamage, int64_t startDamage, uint32_t tickInterval)
{
...
bool Monsters::deserializeSpell(xmlNodePtr node, spellBlock_t& sb, const std::string& description)
{
	...
	else
	{
			...
			for(xmlNodePtr tmpNode = node->children; tmpNode; tmpNode = tmpNode->next)
			{
				...
				if(readXMLInteger(tmpNode, "type", intValue))
				{
					...
					if(readXMLInteger(tmpNode, "oldType", intValue)) {
						g_game.setOldType(outfit.lookType, intValue);
					}
			...
			tmpName == "drowncondition")
		{
			...
			int64_t int64Value;
			int64_t startDamage = 0, minDamage = std::abs(sb.minCombatValue), maxDamage = std::abs(sb.maxCombatValue);
			if(readXMLInteger64(node, "start", int64Value))
				startDamage = std::max(std::abs(int64Value), minDamage);
...
bool Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading/* = false*/)
{
	...
	for(xmlNodePtr p = root->children; p; p = p->next)
	{
		...
		{
			int64_t monsterHealth;
			if(readXMLInteger64(p, "min", monsterHealth))
				mType->healthMin = monsterHealth;

			if(!readXMLInteger64(p, "max", monsterHealth))
			{
			...
			mType->healthMax = monsterHealth;
			if(!readXMLInteger64(p, "now", monsterHealth))
				mType->health = mType->healthMax;
			else
				mType->health = monsterHealth;
		}
		...
		else if(!xmlStrcmp(p->name, (const xmlChar*)"look"))
		{
			if(readXMLInteger(p, "type", intValue))
			{
				...
				if(readXMLInteger(p, "oldType", intValue)) {
					g_game.setOldType(mType->outfit.lookType, intValue);
				}
			}
