	...
	OperatingSystem_t operatingSystem, uint16_t version, bool gamemaster)
{
		...
		(accountManager && name == "Account Manager"))
	{
		...
		if(!player->hasFlag(PlayerFlag_CanAlwaysLogin))
		{
			...
			{
				disconnectClient(0x14, "servidor esta fechado, e voltara dia 21/05.");
				return;
			}
		}
...
void ProtocolGame::sendGoods(const ShopInfoList& shop)
{
	...
	if (Npc* shopOwner = player->shopOwner) {
		uint16_t coinId = shopOwner->getCoinId();
		if (coinId > 0) {
			msg->add<uint32_t>(player->__getItemTypeCount(coinId));
		} else {
			uint64_t money = g_game.getMoney(player) + player->getBankBalance();
			if (money >= INT32_MAX) {
				msg->add<uint32_t>(INT32_MAX -1);
			} else {
				msg->add<uint32_t>((uint32_t)g_game.getMoney(player) + player->getBankBalance());
			}
		}
	} else {
		std::cout << "Warning ProtocolGame::sendSaleItemList shopOwner == nullptr" << std::endl;
		msg->add<uint32_t>(0);
	}
